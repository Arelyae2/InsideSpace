    ...
    public static class MVsToolkitHierarchyEditor
    {
            ...
            else
            {
                switch(state)
                {
                    ...
                    case GameObjectState.ErrorFromMissingComponent:
                        GUI.color = StrToColor(MVsHierarchyPreferences.GetValues().MissingPrefabColor) * (activeSelf ? Color.white : gray7);
                        break;

                    case GameObjectState.Prefab:
                    case GameObjectState.PrefabVariant:
                    case GameObjectState.PrefabChildren:
                        GUI.color = StrToColor(MVsHierarchyPreferences.GetValues().PrefabColor) * (activeSelf ? Color.white : gray7);
                        break;
                }
            }
        ...
        static void DrawGameObjectIcon(Rect rect, Component comp)
        {
            ...
            switch(state)
            {
                ...
                case GameObjectState.Normal:
                case GameObjectState.PrefabChildren:
                    if (MVsHierarchyPreferences.GetValues().DrawFolderIcon && comp == null)
                    {
            ...
            if (MVsHierarchyPreferences.GetValues().DrawFirstComponentIcon && comp != null)
            {
        ...
        static void DrawComponentsIcon(Rect rect, GameObject go, out Component[] comps, out bool haveMissingComponent)
        {
            ...
            for (int i = 1; i < comps.Length; i++)
            {
                ...
                if (comps[i] == null)
                {
                    ...
                    if (MVsHierarchyPreferences.GetValues().DrawComponentsIcon)
                    {
        ...
        static void DrawChildLine(Rect rect)
        {
            ...
            if (MVsHierarchyPreferences.GetValues().DrawChildLines)
            {
        ...
        public static Color HierarchyBackgroundColor(bool isOdd = false)
        {
            ColorUtility.TryParseHtmlString(EditorGUIUtility.isProSkin ?
                            MVsHierarchyPreferences.GetValues().ZebraModBlackColor : MVsHierarchyPreferences.GetValues().ZebraModWhiteColor,
                            out Color c);
            Color color = c;

            if (EditorGUIUtility.isProSkin)
                return (MVsHierarchyPreferences.GetValues().DrawZebraMod && isOdd) ?
